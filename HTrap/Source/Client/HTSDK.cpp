...
#endif
					Internals::g_lastError = HACKTRAP_ERROR_SECURITY;
...
bool HackTrap::Initialize(const std::string& license, HackTrap::HackTrapCallback callback)
{
	...
	// Set the config
	return Internals::SaveConfig(license, reinterpret_cast<void*>(callback));
}
...
bool HackTrap::BasicHeartbeat::BeginProtection()
{
	...
	std::thread guardThread([]() {
		...
		while (true)
		{
			...
			if (!g_armed && local != HACKTRAP_MAGIC_VALUE)
			{
#ifdef HACKTRAP_DEBUG
				MessageBoxA(nullptr, "guardThread fired!", "HackTrap", 0);
#endif
