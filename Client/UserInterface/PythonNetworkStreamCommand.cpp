...
void CPythonNetworkStream::ServerCommand(char * c_szCommand)
{
	...
	else if (!strcmpi(szCmd, "shop_search"))
	{
		if ("clear" == TokenVector[1])
		{
			CPythonPlayer::instance().ClearMapShopSearch();
		}
	}
...
//	}
	...
	//	}
		...
		//	);
	//}
	else if (!strcmpi(szCmd, "OpenOfflineShop"))
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflineShop_OpenOfflineShop", Py_BuildValue("()"));
	}
	else if (!strcmpi(szCmd, "CloseOfflineShopBuilderWindow"))
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflineShop_CloseOfflineShopBuilderWindow", Py_BuildValue("()"));
	}
#endif
