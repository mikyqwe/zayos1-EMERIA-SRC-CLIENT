...
class CPythonPlayer: public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	public:
		...
		typedef struct SPlayerStatus
		{
			...
			TItemData			aSkillBookItem[c_Special_Storage_ItemSlot_Count];
			TItemData			aUppItem[c_Special_Storage_ItemSlot_Count];
			TItemData			aGhostStoneItem[c_Special_Storage_ItemSlot_Count];
			TItemData			aGeneralItem[c_Special_Storage_ItemSlot_Count];
#endif
		...
		// SHOP_SEARCH_GRIMM
		typedef struct SShopSearchItems
		{
			char	szNameOwner[CHARACTER_NAME_MAX_LEN + 1];
			DWORD	dwVid;
			DWORD	dwVnum;
			int		iCount;
			int		iRefine;
...
// #endif
	...
	public:
		...
		void		SetCurrentOfflineShopMoney(long long llMoney, int bCheque);
#else
...
#ifdef ENABLE_CHEQUE_SYSTEM
		int		GetCurrentOfflineShopCheque();
#endif
#endif
		...
		// SHOP_SEARCH_GRIMM	
		int		GetItemShopSearch(int index, int info);
		void	RemoveItemShopSearch(int index);
		bool	CanItemShopSearch(int index);
		const char *	GetItemShopSearchChar(int index, int info);
		void	ClearMapShopSearch();
		DWORD	GetItemShopSearchSocket(int index, DWORD dwMetinSocketIndex);
		void	GetItemShopSearchAttribute(int index, DWORD dwAttrSlotIndex, BYTE * pbyType, short * psValue, bool * psFrozen);
		void	AppendItemShopSearch(char szNameOwner[CHARACTER_NAME_MAX_LEN + 1], DWORD dwVid, DWORD dwVnum, int iCount, int iRefine, int iPrice, BYTE bCheque, int iLevel, int value1, int value2, DWORD dwTransmutation, int iPos);
		// SHOP_SEARCH_GRIMM_END
...
#else
		void	SetItemAttribute(TItemPos Cell, DWORD dwAttrIndex, BYTE byType, short sValue, bool isFrozen);
#endif // !1
		...
		DWORD 	GetItemCountByVnumNew(DWORD dwVnum);
...
#else
		void	GetItemAttribute(TItemPos Cell, DWORD dwAttrSlotIndex, BYTE * pbyType, short * psValue, bool * psFrozen);
#endif // !ENABLE
		...
		void 	UpdatePartyMemberInfo(DWORD dwPID, BYTE byState, BYTE byHPPercentage, BYTE bySPPercentage);
	...
	protected:
		...
		// Shop Search 
		std::map<int, TShopSearchItems> TMapShopSearchItems;
...
#ifdef ENABLE_CHEQUE_SYSTEM
		int					m_bCurrentOfflineShopCheque;
#endif
#endif
