...
const TItemData * CPythonPlayer::GetItemData(TItemPos Cell) const
{
	...
	case SWITCHBOT:
		...
		case UPPITEM_INVENTORY:
			return & m_playerStatus.aUppItem[Cell.cell];

		case GHOSTSTONE_INVENTORY:
			return & m_playerStatus.aGhostStoneItem[Cell.cell];

		case GENERAL_INVENTORY:
			return & m_playerStatus.aGeneralItem[Cell.cell];
#endif
...
void CPythonPlayer::SetItemData(TItemPos Cell, const TItemData & c_rkItemInst)
{
	...
	case SWITCHBOT:
		...
		case UPPITEM_INVENTORY:
			m_playerStatus.aUppItem[Cell.cell] = c_rkItemInst;
			break;

		case GHOSTSTONE_INVENTORY:
			m_playerStatus.aGhostStoneItem[Cell.cell] = c_rkItemInst;
			break;

		case GENERAL_INVENTORY:
			m_playerStatus.aGeneralItem[Cell.cell] = c_rkItemInst;
			break;
#endif
	}
}
...
DWORD CPythonPlayer::GetItemCountByVnumNew(DWORD dwVnum)
{
	DWORD dwCount = 0;

	for (int i = 0; i < c_Special_Storage_ItemSlot_Count; ++i)
	{
		const TItemData & c_rItemData = m_playerStatus.aUppItem[i];
		if (c_rItemData.vnum == dwVnum)
		{
			dwCount += c_rItemData.count;
		}
...
#else
void CPythonPlayer::GetItemAttribute(TItemPos Cell, DWORD dwAttrSlotIndex, BYTE * pbyType, short * psValue, bool * psFrozen)
{
	*pbyType = 0;
	*psValue = 0;
	*psFrozen = 0;
	...
	*pbyType = GetItemData(Cell)->aAttr[dwAttrSlotIndex].bType;
	*psValue = GetItemData(Cell)->aAttr[dwAttrSlotIndex].sValue;
	*psFrozen = GetItemData(Cell)->aAttr[dwAttrSlotIndex].isFrozen;
}
#endif
...
#else
void CPythonPlayer::SetItemAttribute(TItemPos Cell, DWORD dwAttrIndex, BYTE byType, short sValue, bool isFrozen)
{
	if (!Cell.IsValidCell())
		return;
	if (dwAttrIndex >= ITEM_ATTRIBUTE_SLOT_MAX_NUM)
	...
	(const_cast <TItemData *>(GetItemData(Cell)))->aAttr[dwAttrIndex].bType = byType;
	(const_cast <TItemData *>(GetItemData(Cell)))->aAttr[dwAttrIndex].sValue = sValue;
	(const_cast <TItemData *>(GetItemData(Cell)))->aAttr[dwAttrIndex].isFrozen = isFrozen;
}
#endif
...
void CPythonPlayer::SetCurrentOfflineShopMoney(long long llMoney, int bCheque)
#else
...
#ifdef ENABLE_CHEQUE_SYSTEM
	m_bCurrentOfflineShopCheque = bCheque;
#endif
}
...
int CPythonPlayer::GetCurrentOfflineShopCheque()
{
	return m_bCurrentOfflineShopCheque;
}
#endif

// SHOP_SEARCH_GRIMM

const char * CPythonPlayer::GetItemShopSearchChar(int index, int info)
{
	if (TMapShopSearchItems.find(index) == TMapShopSearchItems.end())
		return "NO_INFO";
	
	if (info == 0)
		return TMapShopSearchItems[index].szNameOwner;
	
	return "NO_INFO";
}

DWORD CPythonPlayer::GetItemShopSearchSocket(int index, DWORD dwMetinSocketIndex)
{
	if (dwMetinSocketIndex >= ITEM_SOCKET_SLOT_MAX_NUM)
		return 0;

	return TMapShopSearchItems[index].alSockets[dwMetinSocketIndex];
}

void CPythonPlayer::GetItemShopSearchAttribute(int index, DWORD dwAttrSlotIndex, BYTE * pbyType, short * psValue, bool * psFrozen)
{
	*pbyType = 0;
	*psValue = 0;
	*psFrozen = 0;
	...
	*pbyType = TMapShopSearchItems[index].aAttr[dwAttrSlotIndex].bType;
	*psValue = TMapShopSearchItems[index].aAttr[dwAttrSlotIndex].sValue;
	*psFrozen = TMapShopSearchItems[index].aAttr[dwAttrSlotIndex].isFrozen;
}

void CPythonPlayer::ClearMapShopSearch()
{
	TMapShopSearchItems.clear();
}

bool CPythonPlayer::CanItemShopSearch(int index)
{
	if (TMapShopSearchItems.find(index) == TMapShopSearchItems.end())
		return false;
...
void CPythonPlayer::RemoveItemShopSearch(int index)
{
	if (TMapShopSearchItems.find(index) == TMapShopSearchItems.end())
		return;

	//TMapShopSearchItems.erase(index);

	// Send
	// PyCallClassMemberFunc(m_ppyGameWindow, "CreateSearchedItemList", Py_BuildValue("()"));	
}

int CPythonPlayer::GetItemShopSearch(int index, int info)
{
	if (TMapShopSearchItems.find(index) == TMapShopSearchItems.end())
		return 0;

	if (info == 0)
		return TMapShopSearchItems[index].index;
	else if (info == 1)
		return TMapShopSearchItems[index].dwVid;
	else if (info == 2)
		return TMapShopSearchItems[index].dwVnum;
	else if (info == 3)
		return TMapShopSearchItems[index].iCount;
	else if (info == 4)
		return TMapShopSearchItems[index].iRefine;
	else if (info == 5)
		return TMapShopSearchItems[index].iPrice;
	else if (info == 6)
		return TMapShopSearchItems[index].iLevel;
	else if (info == 7)
		return TMapShopSearchItems[index].dwTransmutation;

	else if (info == 8)
	{
		int ipos = 0;
		std::map<int, TShopSearchItems>::iterator itor = TMapShopSearchItems.begin();
		for (; itor != TMapShopSearchItems.end(); ++itor)
			++ipos;
...
void CPythonPlayer::AppendItemShopSearch(char szNameOwner[CHARACTER_NAME_MAX_LEN + 1], DWORD dwVid, DWORD dwVnum, int iCount, int iRefine, int iPrice, BYTE bCheque, int iLevel, int value1, int value2, DWORD dwTransmutation, int iPos)
{
	TShopSearchItems cache;
	cache.dwVnum = dwVnum;
	cache.dwVid = dwVid;
	cache.iCount = iCount;
	cache.iRefine = iRefine;
	cache.iPrice = iPrice;
	cache.bCheque = bCheque;
	cache.iLevel = iLevel;
	cache.dwTransmutation = dwTransmutation;
	cache.index = iPos;
	...
	memcpy(cache.szNameOwner, szNameOwner, sizeof(cache.szNameOwner));
	...
	TMapShopSearchItems[position] = cache;
	
	// Send
	PyCallClassMemberFunc(m_ppyGameWindow, "CreateSearchedItemList", Py_BuildValue("()"));
}

// SHOP_SEARCH_GRIMM_END
...
void CPythonPlayer::UpdatePartyMemberInfo(DWORD dwPID, BYTE byState, BYTE byHPPercentage, BYTE bySPPercentage)
{
	TPartyMemberInfo * pPartyMemberInfo;
	if (!GetPartyMemberPtr(dwPID, &pPartyMemberInfo))
	{
		TraceError(" CPythonPlayer::UpdatePartyMemberInfo(dwPID=%d, byState=%d, byHPPercentage=%d, bySPPercentage=%d) - Failed to find character", dwPID, byState, byHPPercentage, bySPPercentage);
		return;
	}

	pPartyMemberInfo->byState = byState;
	pPartyMemberInfo->byHPPercentage = byHPPercentage;
	pPartyMemberInfo->bySPPercentage = bySPPercentage;
}
...
CPythonPlayer::CPythonPlayer(void)
{
	...
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(CInstanceBase::AFFECT_RESIST_BUFF), 97));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(CInstanceBase::AFFECT_CRITICAL_BUFF), 98));
	m_kMap_dwAffectIndexToSkillIndex.insert(std::make_pair(int(CInstanceBase::AFFECT_REFLECT_BUFF), 99));
#endif
