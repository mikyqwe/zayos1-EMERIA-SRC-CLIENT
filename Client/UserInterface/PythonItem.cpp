...
bool CPythonItem::GetCloseItem(const TPixelPosition & c_rPixelPosition, DWORD * pdwItemID, DWORD dwDistance, const std::string& myName)
{
	DWORD dwCloseItemID = 0;
	DWORD dwCloseItemDistance = 250 * 250;//Slow mode
#ifdef ULTIMATE_PICKUP
	CItemData * pItemData;
#endif
	TGroundItemInstanceMap::iterator i;
	for (i = m_GroundItemInstanceMap.begin(); i != m_GroundItemInstanceMap.end(); ++i)
	{
		...
		DWORD dwDistance = DWORD(dwxDistance*dwxDistance + dwyDistance*dwyDistance);
#ifdef ULTIMATE_PICKUP
		if (!CItemManager::Instance().SelectItemData(pInstance->dwVirtualNumber))
			continue;
		...
		if (pItemData->GetType() == CItemData::ITEM_TYPE_WEAPON)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_WEAPON))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_BODY)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_ARMOR))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_EAR)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_EAR))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_NECK)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_NECK))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_FOOTS)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_FOOTS))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_SHIELD)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_SHIELD))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_HEAD)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_HEAD))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_WRIST)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_WRIST))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_SKILLBOOK)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_BOOK))
				continue;
		}
		else if (pItemData->GetType() == CItemData::ITEM_TYPE_METIN)
		{
			if (IS_SET(CPythonSystem::Instance().GetPickupFilter(), CPythonSystem::PICKUP_FILTER_STONE))
				continue;
		}
#endif
...
{
	DWORD dwCloseItemDistance = 5000 * 5000;
	CItemData * pItemData;
	TGroundItemInstanceMap::iterator i;
	for (i = m_GroundItemInstanceMap.begin(); i != m_GroundItemInstanceMap.end(); ++i)
	{
