...
void CPythonChat::AppendChat(int iType, const char * c_szChat)
{
	...
	{
		pChatLine->InstanceLang.SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer("d:/ymir work/ui/lang/it.tga"));
		pChatLine->InstanceEmpire.SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer("d:/ymir work/ui/empire/empire_a_256.dds"));
	}
	
	if (iType == CHAT_TYPE_SHOUT)
	{
		std::string isLanguageFlag("/lang/");
		int positionLang = strContent.find(isLanguageFlag);
		
		if (positionLang != std::string::npos)
		{
			strContent = strContent.substr(positionLang + isLanguageFlag.length(), strContent.length());
			positionLang = strContent.find("/");
			std::string c_szLanguage = strContent.substr(0, positionLang);
			
			if (strlen(c_szLanguage.c_str()) > 0)
			{
				char buf[128];
				sprintf(buf, "d:/ymir work/ui/lang/%s.tga", c_szLanguage.c_str());
				pChatLine->InstanceLang.SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer(buf));
			}
			strContent = strContent.substr(positionLang + 1, strContent.length());
		}
...
void CPythonChat::AddChatStack(const char* c_szChat)//Scp1453
{
...
const char* CPythonChat::GetChatStack(BYTE index)//Scp1453
{
...
BYTE CPythonChat::GetChatStackSize()//Scp1453
{
