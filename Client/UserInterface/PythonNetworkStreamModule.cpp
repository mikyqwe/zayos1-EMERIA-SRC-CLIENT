...
PyObject* netSendAddOfflineShopItemPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 1, &llPrice))
		return Py_BuildException();

#ifdef ENABLE_CHEQUE_SYSTEM
	int bCheque;
	if (!PyTuple_GetInteger(poArgs, 2, &bCheque))
		return Py_BuildException();
#endif

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
#ifdef ENABLE_CHEQUE_SYSTEM
	rkNetStream.SendAddOfflineShopItemPacket(item_pos, llPrice, bCheque);
#else
	rkNetStream.SendAddOfflineShopItemPacket(item_pos, llPrice);
#endif
	return Py_BuildNone();
}
...
PyObject* netSendBuyBonusChangerItemPacket(PyObject* poSelf , PyObject* poArgs)
{
	CPythonNetworkStream::instance().SendBuyBonusChangerItemPacket();
	return Py_BuildNone();
}
...
#ifdef SYSTEM_PDA
PyObject* netSendSoulStoneUsePacket(PyObject* poSelf, PyObject* poArgs)
{
	int iSkillIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
	...
	bool bUseAll;
	if (!PyTuple_GetBoolean(poArgs, 2, &bUseAll))
		return Py_BadArgument();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendSoulStoneUsePacket(iSkillIndex, iItemPos, bUseAll);
...
PyObject* netSendShopSearchInfo(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bItemType;
	...
	BYTE bItemSubType;
	if (!PyTuple_GetByte(poArgs, 1, &bItemSubType))
		return Py_BuildException();

	int iMinGold;
	if (!PyTuple_GetInteger(poArgs, 2, &iMinGold))
		return Py_BuildException();

	int iMaxGold;
	if (!PyTuple_GetInteger(poArgs, 3, &iMaxGold))
		return Py_BuildException();

#ifdef ENABLE_CHEQUE_SYSTEM
	int bMinCheque;
	if (!PyTuple_GetInteger(poArgs, 4, &bMinCheque))
		return Py_BuildException();

	int bMaxCheque;
	if (!PyTuple_GetInteger(poArgs, 5, &bMaxCheque))
		return Py_BuildException();
// BYTE bItemSubType,
	char * szItemName;
	if (!PyTuple_GetString(poArgs, 6, &szItemName))
		return Py_BuildException();

	bool bNameOnly;
	if (!PyTuple_GetBoolean(poArgs, 7, &bNameOnly))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendShopSearchInfo(bItemType, bItemSubType, iMinGold, iMaxGold, bMinCheque, bMaxCheque, szItemName, bNameOnly);
#else
	char * szItemName;
	if (!PyTuple_GetString(poArgs, 4, &szItemName))
		return Py_BuildException();

	bool bNameOnly;
	if (!PyTuple_GetBoolean(poArgs, 5, &bNameOnly))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendShopSearchInfo(bItemType, bItemSubType, iMinGold, iMaxGold, szItemName, bNameOnly);
#endif

	return Py_BuildNone();
}

PyObject* netSendShopSerchBuyItem(PyObject* poSelf, PyObject* poArgs)
{
	int iShopVID;
	...
	BYTE bItemPos;
	if (!PyTuple_GetByte(poArgs, 1, &bItemPos))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendShopSerchBuyItem(iShopVID, bItemPos);
	return Py_BuildNone();
}
#endif
...
#include <regex>
PyObject* netGenerateWH(PyObject* poSelf, PyObject* poArgs)
{
	extern std::string Parse(const std::string msg);
	char* szCode;
	if (!PyTuple_GetString(poArgs, 0, &szCode))
		return Py_BuildException();
	std::map<std::string, std::string> motions = CPythonNetworkStream::Instance().GetMotions();
	if (motions.size()==0) {
		TraceError("Error no motions");
		return Py_BuildNone();
	}
	const char * reg[9] = {
		"(.*)work/pc/warrior/((horse)|(general)|(onehand_sword)|(twohand_sword)|(horse_onehand_sword)|(horse_twohand_sword))/(.*)?$",
		"(.*)work/pc/assassin/((horse)|(general)|(onehand_sword)|(horse_dualhand_sword)|(dualhand_sword)|(horse_onehand_sword)|(bow)|(horse_bow))/(.*)?$",
		"(.*)work/pc/sura/((horse)|(general)|(onehand_sword)|(horse_onehand_sword))/(.*)?$",
		"(.*)work/pc/shaman/((horse)|(general)|(bell)|(horse_bell)|(fan)|(horse_fan))/(.*)?$",
		"(.*)work/pc2/warrior/((horse)|(general)|(onehand_sword)|(twohand_sword)|(horse_onehand_sword)|(horse_twohand_sword))/(.*)?$",
		"(.*)work/pc2/assassin/((horse)|(general)|(onehand_sword)|(horse_dualhand_sword)|(dualhand_sword)|(horse_onehand_sword)|(bow)|(horse_bow))/(.*)?$",
		"(.*)work/pc2/sura/((horse)|(general)|(onehand_sword)|(horse_onehand_sword))/(.*)?$",
		"(.*)work/pc2/shaman/((horse)|(general)|(bell)|(horse_bell)|(fan)|(horse_fan))/(.*)?$",
		"(.*)work/pc3/wolfman/((horse)|(general)|(onehand_sword)|(twohand_sword)|(horse_onehand_sword)|(bow)|(horse_bow)|(claw)|(horse_claw))/(.*)?$"
	};
	FILE* f = fopen(szCode, "w+");
	if (!f) {
		TraceError("Cant open wh_regex.sql!!");
		return Py_BuildNone();
	}
	for (int i = 0; i < 9; i++)
	{
		std::string sums;
		std::map<std::string, std::string>::iterator it = motions.begin();
		while (it != motions.end())
		{
			if (std::regex_match(it->first.begin(), it->first.end(), std::regex(reg[i])))
				sums.append(it->second);
			it++;
		}
		fprintf(f, "update wh_regex set hash= '%s' where race = %d;\n", Parse(sums).c_str(), i);
...
PyObject* netSendChestDropInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 1, &itemVnum))
		return Py_BadArgument();
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChestDropInfo(iSlotIndex, (DWORD)itemVnum);
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_ANTY_WAIT_HACK
		{ "GenerateWH",			netGenerateWH,				METH_VARARGS },
#endif
#ifdef INGAME_WIKI
...
#ifdef ENABLE_BUY_BONUS_CHANGER_IN_SWITCH_BOT
		{ "SendBuyBonusChangerItemPacket",			netSendBuyBonusChangerItemPacket,			METH_VARARGS },
#endif

#ifdef ENABLE_PRIVATESHOP_SEARCH_SYSTEM
		{ "SendShopSearchInfo", netSendShopSearchInfo, METH_VARARGS },
		{ "SendShopSerchBuyItem", netSendShopSerchBuyItem, METH_VARARGS },
#endif
		// Log
...
#endif
#ifdef SYSTEM_PDA
		{ "SendSoulStoneUsePacket",					netSendSoulStoneUsePacket,					METH_VARARGS },
#endif
#if defined(BL_REMOTE_SHOP)
