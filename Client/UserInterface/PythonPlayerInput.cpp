...
void CPythonPlayer::PickCloseItem()
{
	...
	DWORD dwItemID;
	CPythonItem& rkItem=CPythonItem::Instance();
	if (!rkItem.GetCloseItem(kPPosMain, &dwItemID, __GetPickableDistance(), pkInstMain->GetNameString()))
		return;
...
void CPythonPlayer::NEW_Fishing()
{
	...
	if (pkInstMain->IsFishing()) {
		...
		if (pkInstMain->CanFishing()) {
			int r;
			if (pkInstMain->GetFishingRot(&r)) {
				CPythonNetworkStream::Instance().SendFishingPacketNew(r, 1);
			}
			else {
...
{
#ifdef ENABLE_AUTO_ATTACK_RENEW
