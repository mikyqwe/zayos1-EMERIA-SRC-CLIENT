...
std::map<int, CGraphicImage *> m_kMap_iSpecialStatsIndex_pkIconImage;
#endif // 
...
// SHOP SEARCH GRIMM
PyObject * playerGetShopSearchItem(PyObject * poSelf, PyObject * poArgs)
{
	int index;
	if (!PyTuple_GetInteger(poArgs, 0, &index))
		return Py_BuildException();

	return Py_BuildValue("siiiiiiii", 
	CPythonPlayer::Instance().GetItemShopSearchChar(index, 0),
	CPythonPlayer::Instance().GetItemShopSearch(index, 0), 
	CPythonPlayer::Instance().GetItemShopSearch(index, 1),
	CPythonPlayer::Instance().GetItemShopSearch(index, 2),
	CPythonPlayer::Instance().GetItemShopSearch(index, 3),
	CPythonPlayer::Instance().GetItemShopSearch(index, 4),
	CPythonPlayer::Instance().GetItemShopSearch(index, 5),
	CPythonPlayer::Instance().GetItemShopSearch(index, 6),
	CPythonPlayer::Instance().GetItemShopSearch(index, 7));
}

PyObject * playerGetShopSearchItemAttribute(PyObject * poSelf, PyObject * poArgs)
{
	int index;
	if (!PyTuple_GetInteger(poArgs, 0, &index))
		return Py_BuildException();
	...
	short sValue;
	bool isFrozen;
	CPythonPlayer::Instance().GetItemShopSearchAttribute(index, iAttributeSlotIndex, &byType, &sValue, &isFrozen);
	return Py_BuildValue("iii", byType, sValue, isFrozen);
}


PyObject * playerGetShopSearchItemSocket(PyObject * poSelf, PyObject * poArgs)
{
	int index;
	if (!PyTuple_GetInteger(poArgs, 0, &index))
		return Py_BuildException();
	...
	int nMetinSocketValue = CPythonPlayer::Instance().GetItemShopSearchSocket(index, iMetinSocketIndex);
	return Py_BuildValue("i", nMetinSocketValue);
}

PyObject * playerCanItemShopSearch(PyObject * poSelf, PyObject * poArgs)
{
	int index;
	if (!PyTuple_GetInteger(poArgs, 0, &index))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonPlayer::Instance().CanItemShopSearch(index));
}

PyObject * playerRemoveItemShopSearch(PyObject * poSelf, PyObject * poArgs)
{
	int index;
	if (!PyTuple_GetInteger(poArgs, 0, &index))
		return Py_BuildException();
	
	CPythonPlayer::Instance().RemoveItemShopSearch(index);
}

PyObject * playerGetShopSearchSize(PyObject * poSelf, PyObject * poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetItemShopSearch(0, 8));
}
// SHOP SEARCH GRIMM
...
PyObject * playerGetItemCountByVnumNew(PyObject* poSelf, PyObject* poArgs)
{
	...
	int ItemNum = CPythonPlayer::Instance().GetItemCountByVnumNew(ivnum);
	return Py_BuildValue("i", ItemNum);
}
...
PyObject * playerGetItemAttribute(PyObject* poSelf, PyObject* poArgs)
{
	...
	short sValue;
	bool isFrozen;
	CPythonPlayer::Instance().GetItemAttribute(Cell, iAttributeSlotIndex, &byType, &sValue, &isFrozen);
...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		for (int i = 0; i < ITEM_ATTRIBUTE_SLOT_MAX_NUM; ++i)
		{
			...
			{
				len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%d:%d",
						pPlayerItem->aAttr[i].bType, pPlayerItem->aAttr[i].sValue, pPlayerItem->aAttr[i].isFrozen);
				isAttr = true;
			}
...
PyObject * playerGetAllItemCount(PyObject* poSelf, PyObject* poArgs)
{
	DWORD dwVnum;
	if (!PyTuple_GetLong(poArgs, 0, (long*)&dwVnum))
		return Py_BuildException();

	DWORD ItemNum = CPythonPlayer::Instance().GetItemCountByVnum(dwVnum);
	ItemNum += CPythonPlayer::Instance().GetItemCountByVnumNew(dwVnum);
...
PyObject * playerGetPartyMemberSPPercentage(PyObject* poSelf, PyObject* poArgs)
{
	int iPID;
	if (!PyTuple_GetInteger(poArgs, 0, &iPID))
		return Py_BuildException();
	...
	return Py_BuildValue("i", pPartyMemberInfo->bySPPercentage);
}
...
#ifdef ENABLE_SPECIAL_STATS_SYSTEM
PyObject * playerRegisterSpecialStatsIcon(PyObject* poSelf, PyObject* poArgs) {
	...
	m_kMap_iSpecialStatsIndex_pkIconImage.insert(std::make_pair(iIndex, pImage));

	return Py_BuildNone();
}

PyObject * playerGetSpecialStatsIconImage(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();

	if (m_kMap_iSpecialStatsIndex_pkIconImage.end() == m_kMap_iSpecialStatsIndex_pkIconImage.find(iIndex))
		return Py_BuildValue("i", 0);

	return Py_BuildValue("i", m_kMap_iSpecialStatsIndex_pkIconImage[iIndex]);
}
...
PyObject * playerSetItemAttribute(PyObject* poSelf, PyObject* poArgs)
{
	...
	case 6:
		...
		if (!PyTuple_GetInteger(poArgs, 5, &iAttributeFrozen))
			return Py_BuildException();
		break;
#endif // ENABLE_FROZEN_SYSTEM
	}
...
#else
	CPythonPlayer::Instance().SetItemAttribute(ItemPos, iAttributeSlotIndex, iAttributeType, iAttributeValue, iAttributeFrozen);
#endif // ENABLE_FROZEN_SYSTEM
...
#ifdef ___DECORUM___
PyObject * playerGetDecorum(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase * pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	if (!pInstance)
		return Py_BuildValue("i", 0);

	return Py_BuildValue("i", pInstance->GetDecorumLegue());
}

PyObject * playerIsDecored(PyObject* poSelf, PyObject* poArgs)
{
	int iVID;
	if (!PyTuple_GetInteger(poArgs, 0, &iVID))
		return Py_BuildException();
	...
	return Py_BuildValue("i", pTargetInstance->GetDecorum() > 0);
}
#endif
...
PyObject * playerCanMoveItem(PyObject* poSelf, PyObject* poArgs)
{
	int iItemVnum;
	if (!PyTuple_GetInteger(poArgs, 0, &iItemVnum))
		return Py_BuildValue("i", 0);
	switch ((DWORD)iItemVnum)
	{
		...
		case 30183: case 30188: case 72322: case 31036: case 39063:
		case 39064: case 39065: case 39066: case 39067: case 39068: case 39069:
		case 70022:
			return Py_BuildValue("i", 1);
	}
	
	if (CItemManager::Instance().SelectItemData((DWORD)iItemVnum))
	{
		CItemData * pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
		if (pItemData)
		{
			BYTE itemType = pItemData->GetType();
			BYTE itemSubType = pItemData->GetSubType();
			if (itemType == CItemData::ITEM_TYPE_SKILLBOOK){
				return Py_BuildValue("i", 1);
			}
			else if (itemType == CItemData::ITEM_TYPE_METIN && itemSubType == CItemData::METIN_NORMAL){
				return Py_BuildValue("i", 1);
			}
			else if (itemType == CItemData::ITEM_TYPE_USE)
			{
				if (itemSubType == CItemData::USE_CHANGE_ATTRIBUTE
					|| itemSubType == CItemData::USE_ADD_ATTRIBUTE
					|| itemSubType == CItemData::USE_ADD_ATTRIBUTE2
					|| itemSubType == CItemData::USE_CHANGE_COSTUME_ATTR
					|| itemSubType == CItemData::USE_RESET_COSTUME_ATTR
					|| itemSubType == CItemData::USE_TUNING)
					return Py_BuildValue("i", 1);
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetAllItemCount",			playerGetAllItemCount,				METH_VARARGS },
...
#ifdef ENABLE_SPECIAL_STATS_SYSTEM
		{ "RegisterSpecialStatIcon",	playerRegisterSpecialStatsIcon,		 METH_VARARGS },
		{ "GetSpecialStatIconImage",	 playerGetSpecialStatsIconImage,	 METH_VARARGS },
#endif
...
#endif
#ifdef ___DECORUM___
		{ "GetDecorum",					playerGetDecorum,					METH_VARARGS },
		{ "IsDecored",					playerIsDecored,					METH_VARARGS },
#endif
		{ "CanMoveItem", playerCanMoveItem, METH_VARARGS },

		// Shop search grimm
		{ "RemoveItemShopSearch",		playerRemoveItemShopSearch,			METH_VARARGS },
		{ "CanItemShopSearch",			playerCanItemShopSearch,			METH_VARARGS },
		{ "GetShopSearchItem",			playerGetShopSearchItem,			METH_VARARGS },
		{ "GetShopSearchSize",			playerGetShopSearchSize,			METH_VARARGS },
		{ "GetShopSearchItemSocket",	playerGetShopSearchItemSocket,		METH_VARARGS },
		{ "GetShopSearchItemAttribute",	playerGetShopSearchItemAttribute,	METH_VARARGS },
#ifdef GUILD_WAR_COUNTER
	...
	PyModule_AddIntConstant(poModule, "SLOT_TYPE_GHOSTSTONE_INVENTORY",			SLOT_TYPE_GHOSTSTONE_INVENTORY);
	...
	PyModule_AddIntConstant(poModule, "GHOSTSTONE_INVENTORY",					GHOSTSTONE_INVENTORY);
	...
	PyModule_AddIntConstant(poModule, "PVP_CRITICAL_DAMAGE_SKILLS", PVP_CRITICAL_DAMAGE_SKILLS);
	PyModule_AddIntConstant(poModule, "PVP_POISONING", PVP_POISONING);
	PyModule_AddIntConstant(poModule, "PVP_HALF_HUMAN", PVP_HALF_HUMAN);
	PyModule_AddIntConstant(poModule, "PVP_BUFFI_SKILLS", PVP_BUFFI_SKILLS);
	PyModule_AddIntConstant(poModule, "PVP_MISS_HITS", PVP_MISS_HITS);
	PyModule_AddIntConstant(poModule, "PVP_DISPEL_EFFECTS", PVP_DISPEL_EFFECTS);
	PyModule_AddIntConstant(poModule, "PVP_HP_ELIXIR", PVP_HP_ELIXIR);
	PyModule_AddIntConstant(poModule, "PVP_WHITE_DEW", PVP_WHITE_DEW);
	PyModule_AddIntConstant(poModule, "PVP_YELLOW_DEW", PVP_YELLOW_DEW);
	PyModule_AddIntConstant(poModule, "PVP_ORANGE_DEW", PVP_ORANGE_DEW);
	PyModule_AddIntConstant(poModule, "PVP_RED_DEW", PVP_RED_DEW);
	PyModule_AddIntConstant(poModule, "PVP_BLUE_DEW", PVP_BLUE_DEW);
	PyModule_AddIntConstant(poModule, "PVP_GREEN_DEW", PVP_GREEN_DEW);
	PyModule_AddIntConstant(poModule, "PVP_ZIN_WATER", PVP_ZIN_WATER);
	PyModule_AddIntConstant(poModule, "PVP_SAMBO_WATER", PVP_SAMBO_WATER);
	PyModule_AddIntConstant(poModule, "PVP_ATTACKSPEED_FISH", PVP_ATTACKSPEED_FISH);
	PyModule_AddIntConstant(poModule, "PVP_DRAGON_GOD_ATTACK", PVP_DRAGON_GOD_ATTACK);
	PyModule_AddIntConstant(poModule, "PVP_DRAGON_GOD_DEFENCE", PVP_DRAGON_GOD_DEFENCE);
	PyModule_AddIntConstant(poModule, "PVP_DRAGON_GOD_LIFE", PVP_DRAGON_GOD_LIFE);
	PyModule_AddIntConstant(poModule, "PVP_PIERCING_STRIKE", PVP_PIERCING_STRIKE);
	PyModule_AddIntConstant(poModule, "PVP_CRITICAL_STRIKE", PVP_CRITICAL_STRIKE);
	PyModule_AddIntConstant(poModule, "PVP_PET", PVP_PET);
	PyModule_AddIntConstant(poModule, "PVP_NEW_PET", PVP_NEW_PET);
	PyModule_AddIntConstant(poModule, "PVP_ENERGY", PVP_ENERGY);
	PyModule_AddIntConstant(poModule, "PVP_BET", PVP_BET);
	PyModule_AddIntConstant(poModule, "PVP_MAX", PVP_MAX);
#endif
	...
	PyModule_AddIntConstant(poModule, "KILL_MONSTERS", KILL_MONSTERS);
...
#endif	
#ifdef ENABLE_SPECIAL_STORAGE_SYSTEM
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_PAGE_SIZE",	c_Special_Storage_Inventory_Page_Size);
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_PAGE_COUNT",	c_Special_Storage_Inventory_Page_Count);
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_SLOT_COUNT",	c_Special_Storage_ItemSlot_Count);
#endif
