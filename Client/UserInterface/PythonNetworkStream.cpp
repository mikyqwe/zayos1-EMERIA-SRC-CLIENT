...
class CMainPacketHeaderMap : public CNetworkPacketHeaderMap
{
	...
	public:
		CMainPacketHeaderMap()
		{
			...
			Set(HEADER_GC_ANTY_WAIT_HACK, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCAntyWH), false));
#endif
...
#endif
#ifdef ___DECORUM___
			Set(HEADER_GC_DECORUM_STAT, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCDecorumStat), STATIC_SIZE_PACKET));
			Set(HEADER_GC_DECORUM_BASE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCDecorumBase), STATIC_SIZE_PACKET));
			Set(HEADER_GC_DECORUM_BATTLE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCDecorumBattle), STATIC_SIZE_PACKET));
			Set(HEADER_GC_DECORUM_KILLS, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCDecorumKills), STATIC_SIZE_PACKET));
#endif
			...
			Set(HEADER_CG_REQUEST_FLAG,	CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketCGRequestFlag), STATIC_SIZE_PACKET));
#endif
			...
			Set(HEADER_GC_67_BONUS_NEW, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGC67BonusReceive), STATIC_SIZE_PACKET));
#endif
			...
			Set(HEADER_GC_PSHOP_SEARCH_ITEM_INFO, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCShopSearchItemInfo), STATIC_SIZE_PACKET));
#endif
...
#endif
			Set(HEADER_GC_SEARCH_SYSTEM, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCShopSearchSystem), STATIC_SIZE_PACKET));
...
#ifdef ENABLE_ANTY_WAIT_HACK
void CPythonNetworkStream::LoadMotion(std::string name, const char * race)
{
	motions[name] = race;
}
void LoadMotion(const char * name, const char * race)
{

	CPythonNetworkStream::Instance().LoadMotion(name, race);
}
#endif
