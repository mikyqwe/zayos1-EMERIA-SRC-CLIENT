...
#else
		rkPlayer.SetItemAttribute(packet_item_update.Cell, j, packet_item_update.aAttr[j].bType, packet_item_update.aAttr[j].sValue, packet_item_update.aAttr[j].isFrozen);
#endif
...
bool CPythonNetworkStream::SendAddOfflineShopItemPacket(TItemPos item_pos, int llPrice, int bCheque)
#else
bool CPythonNetworkStream::SendAddOfflineShopItemPacket(TItemPos item_pos, int llPrice)
#endif
// END_OF_MAX_YANG
{
	if (!__CanActMainInstance())
	...
	// BEGIN_MAX_YANG
	pTable.price = llPrice;
	// END_OF_MAX_YANG
#ifdef ENABLE_CHEQUE_SYSTEM
	pTable.cheque = bCheque;
#endif
...
bool CPythonNetworkStream::SendRefreshOfflineShopMoney()
{
	TPacketCGShop PacketShop;
	PacketShop.header = HEADER_CG_OFFLINE_SHOP;
	PacketShop.subheader = SHOP_SUBHEADER_CG_REFRESH_OFFLINE_SHOP_MONEY;
...
bool CPythonNetworkStream::RecvSpecialEffect()
{
	...
	switch (kSpecialEffect.type)
	{
		...
		case SE_OVER_KILL_START:
			effect = CInstanceBase::EFFECT_OVER_KILL_START;
			break;

		case SE_OVER_KILL_START +1:
			effect = CInstanceBase::EFFECT_OVER_KILL_START+1;
			break;

		case SE_OVER_KILL_START + 2:
			effect = CInstanceBase::EFFECT_OVER_KILL_START + 2;
			break;
		case SE_OVER_KILL_START + 3:
			effect = CInstanceBase::EFFECT_OVER_KILL_START + 3;
			break;

		case SE_OVER_KILL_END:
			effect = CInstanceBase::EFFECT_OVER_KILL_END;
			break;
#endif
