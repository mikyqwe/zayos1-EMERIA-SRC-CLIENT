...
class CPythonChat : public CSingleton<CPythonChat>, public IAbstractChat
{
	public:
		...
		enum
		{
			...
			CHAT_STACK_MAX_NUM = 200,//Scp1453 BYTE
		...
		std::vector<std::string>	m_WritedChatStack;//Scp1453
	public:
		CPythonChat();
		virtual ~CPythonChat();
		...
		void AddChatStack(const char* c_szChat); //Scp1453
		const char* GetChatStack(BYTE index); //Scp1453
		BYTE GetChatStackSize(); //Scp1453
