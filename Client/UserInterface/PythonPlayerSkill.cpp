...
bool CPythonPlayer::__HasItem(DWORD dwItemID)
{
	...
	for (int j = 0; j < c_Special_Storage_ItemSlot_Count; ++j)
	{
		if (dwItemID == GetItemIndex(TItemPos(SKILLBOOK_INVENTORY, j)))
			return true;

		if (dwItemID == GetItemIndex(TItemPos(UPPITEM_INVENTORY, j)))
			return true;

		if (dwItemID == GetItemIndex(TItemPos(GHOSTSTONE_INVENTORY, j)))
			return true;
