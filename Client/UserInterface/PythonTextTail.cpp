...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		CGraphicImageInstance* pLanguageInstance = pTextTail->pLanguageImageInstance;
#endif
		...
		CGraphicTextInstance * pLevel;
		int iLevelPosX = 0, iLevelWidth = 0, iLevelHeight = 0;
		int iTitleWidth = 0, iTitleHeight = 0;
		int iMhLevelWidth = 0, iMhLevelHeight = 0;
		...
		// Level
		pLevel = pTextTail->pLevelTextInstance;
		...
		if (pLevel)
		{
			...
			if (LocaleService_IsEUROPE())
				iLevelPosX = pTextTail->x - (iNameWidth / 2) - iTitleWidth;
			else
				iLevelPosX = pTextTail->x - (iNameWidth / 2) - fxAdd - iTitleWidth;

			pLevel->SetPosition(iLevelPosX - fxAdd2, pTextTail->y, pTextTail->z);
			pLevel->Update();
		}
...
#ifdef ___DECORUM___
		if (pTextTail->pDecorumInstance)
			pTextTail->pDecorumInstance->Render();
#endif		
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
...
#define LEADER_GUILD_COLOR    "|cFFd0ffcc" //Change color from http://www.color-hex.com/color/ccffff
...
void CPythonTextTail::ShowAllTextTail()
{
	...
	for (itor = m_ChatTailMap.begin(); itor != m_ChatTailMap.end(); ++itor)
	{
		TTextTail * pTextTail = itor->second;
		if (pTextTail->fDistanceFromPlayer < 3500.0f) {
			pTextTail->pTextInstance->SetOutline(CPythonSystem::Instance().IsAlwaysShowName() != 3);
		}
	}
}
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
...
#endif
#ifdef ___DECORUM___
	pTextTail->pDecorumInstance = NULL;
#endif
	if (0 != dwGuildID)
	{
	...
	{
		const char * c_szLanguageString = pCharacterInstance->GetLanguageString();
		if (*c_szLanguageString)
		{
			char szLanguagePath[256];
			sprintf(szLanguagePath, "d:/ymir work/ui/lang/%s.tga", c_szLanguageString);
...
void CPythonTextTail::DeleteTextTail(TTextTail * pTextTail)
{
	...
	if (pTextTail->pDecorumInstance)
	{
		pTextTail->pDecorumInstance->Destroy();
		pTextTail->pDecorumInstance = NULL;
	}
#endif	
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
...
#ifdef ___DECORUM___
void CPythonTextTail::AttachDecorumLegue(DWORD dwVID, BYTE bLegue)
{
	if (!bLegue)
		return;
	...
	CGraphicImageInstance *& prDecorum = pTextTail->pDecorumInstance;
	if (!prDecorum)
		pTextTail->pDecorumInstance = CGraphicImageInstance::New();

	char buf[256];
	sprintf(buf, "d:/ymir work/ui/universalelements/decorum/universalelements_legue_mini_%d.sub", bLegue);
	pTextTail->pDecorumInstance->SetImagePointer((CGraphicSubImage *)CResourceManager::Instance().GetResourcePointer(buf));
}

void CPythonTextTail::DetachDecorumLegue(DWORD dwVID)
{
	TTextTailMap::iterator itor = m_CharacterTextTailMap.find(dwVID);
	if (m_CharacterTextTailMap.end() == itor)
		return;
	...
	if (pTextTail->pDecorumInstance)
	{
		CGraphicImageInstance::Delete(pTextTail->pDecorumInstance);
		pTextTail->pDecorumInstance = NULL;
	}
}
#endif
