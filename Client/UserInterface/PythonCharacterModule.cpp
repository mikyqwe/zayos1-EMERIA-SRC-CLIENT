...
{
	int iVirtualID;
#ifndef GAMEMASTER_ITEMVAULE_TOOLTIP
	if (!PyTuple_GetInteger(poArgs, 0, &iVirtualID))
		return Py_BuildException();

	CInstanceBase * pInstance = CPythonCharacterManager::Instance().GetInstancePtr(iVirtualID);
#else
	CInstanceBase * pInstance = NULL;
	if (!PyTuple_GetInteger(poArgs, 0, &iVirtualID))
		pInstance = CPythonCharacterManager::Instance().GetSelectedInstancePtr();
	else
		pInstance = CPythonCharacterManager::Instance().GetInstancePtr(iVirtualID);
#endif
...
PyObject * chrAttachEffectByID(PyObject* poSelf, PyObject* poArgs)
{
	...
	pkInst->PublicAttachEffect((UINT)iEffectID);
	return Py_BuildNone();
}
#else
PyObject * chrAttachEffectByID(PyObject* poSelf, PyObject* poArgs)
{
	...
	//pkInst->PublicAttachEffect(iEffectID);
	return Py_BuildNone();
}
#endif
...
PyObject* chrAttachSpecialEffect(PyObject* poSelf, PyObject* poArgs)
{
	...
	int effID;
	if (!PyTuple_GetInteger(poArgs, 1, &effID))
		return Py_BadArgument();
	...
	pCharacterInstance->AttachSpecialEffect(effID);
...
void initchr()
{
...
#endif
		{ "AttachSpecialEffect",			chrAttachSpecialEffect,				METH_VARARGS },
#ifdef GUILD_WAR_COUNTER
	...
	PyModule_AddIntConstant(poModule, "AFFECT_OVER_KILL_FURY", CInstanceBase::AFFECT_OVER_KILL_FURY);
#endif
	...
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_DRAGON_SOUL_DECK_COMPLETE", CInstanceBase::NEW_AFFECT_DRAGON_SOUL_DECK_COMPLETE);
...
#ifdef ENABLE_OVER_KILL
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_OVER_KILL_FURY" , CInstanceBase::NEW_AFFECT_OVER_KILL_FURY);
#endif

#ifdef ___DECORUM___
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_NO_LOST_DECORUM",			CInstanceBase::NEW_AFFECT_NO_LOST_DECORUM);
	PyModule_AddIntConstant(poModule, "NEW_AFFECT_GAIN_DECORUM",			CInstanceBase::NEW_AFFECT_GAIN_DECORUM);
#endif
}
