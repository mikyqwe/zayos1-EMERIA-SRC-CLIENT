...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
		...
		{
			aMarkPosition.m_fX = ( m_fWidth - (float)m_BossMarkE.GetWidth() ) / 2.0f + fDistanceFromCenterX + m_fScreenX;
			aMarkPosition.m_fY = ( m_fHeight - (float)m_BossMarkE.GetHeight() ) / 2.0f + fDistanceFromCenterY + m_fScreenY;
...
void CPythonMiniMap::Render(float fScreenX, float fScreenY)
{
	...
	{
		TMarkPosition & rPosition = *aIterator;
		m_BossMarkE.SetPosition(rPosition.m_fX, rPosition.m_fY);
		m_BossMarkE.Render();
		++aIterator;
	}
#endif	
...
#ifdef BOSS_MARK_SYSTEM
	const std::string strBossMarkE = strImageRoot + "minimap/bossmark.tga";
#endif
...
#ifdef BOSS_MARK_SYSTEM
	pSubImage = (CGraphicSubImage *) CResourceManager::Instance().GetResourcePointer(strBossMarkE.c_str());
	m_BossMarkE.SetImagePointer(pSubImage);
#endif
