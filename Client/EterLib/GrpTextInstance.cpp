...
void CGraphicTextInstance::UpdateMousePos(int x, int y)
{
	gs_mx = x;
	gs_my = y;
	gs_text = "";
...
std::string& CGraphicTextInstance::GetTextFromMousePos()
{
	return gs_text;
}
	...
	while (begin < end)
	{
		...
		else
		{
			if (defCodePage == CP_ARABIC) // ARABIC
			{
				...
				int wArabicTextLen = Arabic_MakeShape(wText.get(), wTextLen, wArabicText, wTextLen);
				...
				for (int i = 0; i < wTextLen; )
				{
					...
					else
					{
						...
						else if (ret == TEXT_TAG_EMOJI_END)
						{
							...
							retBuf[retLen] = '\0';
							char szPath[255];
							sprintf(szPath, "d:/ymir work/ui/emoji/%s.tga", retBuf);
...
void CGraphicTextInstance::Render(RECT* pClipRect, const double scaling)
#else
