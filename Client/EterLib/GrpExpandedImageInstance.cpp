...
void CGraphicExpandedImageInstance::OnRender(const double scaling)
{
	...
	else
	{
		...
		//TODO : check if the offset need to be adjusted by the rotation
...
#ifdef ENABLE_WINDOW_SLIDE_EFFECT
long CGraphicExpandedImageInstance::GetRenderingWidth() {
	auto width = float(GetWidth()) * m_v2Scale.x;
#ifdef INGAME_WIKI
	width += m_RenderingRect.left_top + m_RenderingRect.right_top; // have to correction
#else
	width += m_RenderingRect.left + m_RenderingRect.right;
#endif
	return width;
}

long CGraphicExpandedImageInstance::GetRenderingHeight() {
	auto height = float(GetHeight()) * m_v2Scale.y;
#ifdef INGAME_WIKI
	height += m_RenderingRect.top_left + m_RenderingRect.bottom_left; // have to correction
#else
	height += m_RenderingRect.top + m_RenderingRect.bottom;
#endif
	return height;
}
#endif
...
void CGraphicExpandedImageInstance::RenderCoolTime(float fCoolTime, bool bIsInverse)
{
	if (IsEmpty())
		return;
...
void CGraphicExpandedImageInstance::OnRenderCoolTime(float fCoolTime, bool bIsInverse)
{
	if (fCoolTime >= 1.0f)
		fCoolTime = 1.0f;
	...
	const float su = (c_rRect.left - m_renderBox.left) * texReverseWidth;
	const float sv = (c_rRect.top - m_renderBox.top) * texReverseHeight;
	const float eu = (c_rRect.right + m_renderBox.right) * texReverseWidth;
	const float ev = (c_rRect.bottom + m_renderBox.bottom) * texReverseHeight;

	const float fimgWidth = c_rRect.right - c_rRect.left;
	const float fimgHeight = c_rRect.bottom - c_rRect.top;
	...
	if (fCoolTime < 1.0f)
	{
		...
		vertex.texCoord = TTextureCoordinate(0.5f * (eu + su), 0.5f * (ev + sv));
		vertex.diffuse = m_DiffuseColor;
		vertices.push_back(vertex);

		vertex.position = TPosition(fxCenter, fyCenter - fimgHeightHalf, m_fDepth);
		vertex.texCoord = TTextureCoordinate(0.5f * (eu + su), sv);
		vertex.diffuse = m_DiffuseColor;
		vertices.push_back(vertex);
...
void CGraphicExpandedImageInstance::RenderRectCircle(float fRectCircle, bool bIsInverse)
{
	if (IsEmpty())
		return;
	assert(!IsEmpty());
...
void CGraphicExpandedImageInstance::OnRenderRectCircle(float fRectCircle, bool bIsInverse)
{
	if (fRectCircle >= 1.0f)
		fRectCircle = 1.0f;
