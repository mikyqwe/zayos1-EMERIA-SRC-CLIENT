...
void CRenderTarget::SetArmor(DWORD dwVnum)
{
	if (m_pModelInstance == nullptr || !m_bShow)
		return;

	m_pModelInstance->ChangeArmor(dwVnum);
}

void CRenderTarget::SetWeapon(DWORD dwVnum)
{
	if (m_pModelInstance == nullptr || !m_bShow)
		return;

	m_pModelInstance->ChangeWeapon(dwVnum);
}

void CRenderTarget::SetAcce(DWORD dwVnum)
{
	if (m_pModelInstance == nullptr || !m_bShow)
		return;

	m_pModelInstance->ChangeAcce(dwVnum - 85000);
}

void CRenderTarget::SetAura(DWORD dwVnum)
{
	if (m_pModelInstance == nullptr || !m_bShow)
		return;

	m_pModelInstance->SetAuraRender(dwVnum);
}

void CRenderTarget::ChangeHair(DWORD dwVnum)
{
	if (m_pModelInstance == nullptr || !m_bShow)
		return;

	m_pModelInstance->ChangeHair(dwVnum);
}
...
void CRenderTarget::ChangeEffect()
{
	...
	m_pModelInstance->GetGraphicThingInstanceRef().RenderAllAttachingEffect();
...
void CRenderTarget::SelectModel(const DWORD c_dwIndex)
{
	CInstanceBase::SCreateData kCreateData{};

	kCreateData.m_bType = CActorInstance::TYPE_PC;
	kCreateData.m_dwRace = c_dwIndex;

	auto pModel = std::make_unique<CInstanceBase>();
	...
	m_pModelInstance = std::move(pModel);
	...
	m_pModelInstance->NEW_SetPixelPosition(TPixelPosition(0, 0, 0));
