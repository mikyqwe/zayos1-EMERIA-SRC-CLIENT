...
bool CTextFileLoader::LoadGroup(TGroupNode * pGroupNode)
{
	...
	extern void LoadMotion(const char * name, const char * race);
	std::string lines;
	for (int i = 0; i < m_textFileLoader.GetLineCount(); ++i)
		lines.append(m_textFileLoader.GetLineString(i));
	LoadMotion(m_strFileName.c_str(), Parse(lines).c_str());
#endif
