...
namespace grp {
	...
	long  g_ParentX = 0.0f, g_ParentY = 0.0f;
	long  g_ParentStoredX = 0.0f, g_ParentStoredY = 0.0f;

	float SetSliding(float scaling) {
	...
	bool IsSliding() {
		return g_fScaling != 1.0f;
	}

	void SetParentCoord(long x, long y, long xStored, long yStored)
	{
		g_ParentX = x;
		g_ParentY = y;

		g_ParentStoredX = xStored;
		g_ParentStoredY = yStored;
	}
	...
	void NormalizeBySlidingParent(IN OUT long& x, IN OUT long& y) {
		/*auto local_x = x - g_ParentX;
		auto local_y = y - g_ParentY;

		local_x -= (local_x * (1.0 - g_fScaling)) + 0.5f;
		local_y -= (local_y * (1.0 - g_fScaling)) + 0.5f;

		x = g_ParentX + local_x;
		y = g_ParentY + local_y;*/
	}

	void NormalizeBySlidingParent(IN OUT float& x, IN OUT float& y) {
		/*auto local_x = x - g_ParentX;
		auto local_y = y - g_ParentY;
...
#ifdef ENABLE_WINDOW_SLIDE_EFFECT
	if (grp::IsSliding()) {
		if (grp::GetSliding() == 0.0f)
			return;

		auto w = ex - sx;
		auto h = ey - sy;

		grp::NormalizeBySlidingParent(sx, sy);

		ex = sx + (w * grp::GetSliding());
		ey = sy + (h * grp::GetSliding());
	}
#endif
...
void CScreen::RenderBar3d(float sx, float sy, float sz, float ex, float ey, float ez)
{
	...
	if (grp::IsSliding()) {
		...
		D3DXCOLOR AlphaColor = ms_diffuseColor;
		AlphaColor.a = grp::GetSliding() * grp::GetSliding();
		SetDiffuseColor(AlphaColor);
