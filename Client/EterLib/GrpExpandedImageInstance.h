...
class CGraphicExpandedImageInstance : public CGraphicImageInstance
{
	...
	public:
		...
		void RenderRectCircle(float fCoolTime, bool bIsInverse);

	protected:
		void Initialize();
#ifdef ENABLE_WINDOW_SLIDE_EFFECT
		void OnRender(const double scaling = 1.0);
#else
		void OnRender();
#endif

		
		void OnRenderCoolTime(float fCoolTime, bool bIsInverse);
		void OnRenderRectCircle(float fRectCircle, bool bIsInverse);
