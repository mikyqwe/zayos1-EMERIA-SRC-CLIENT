...
class UI::CSlotWindow::CCoverButton : public CSlotButton
{
	public:
		...
		void RenderButton(double scaling = 1.0)
		{
			CButton::SetScaling(scaling);
			CButton::OnRender();
		}
#else
		void RenderButton()
...
void CSlotWindow::OnRender()
{
	...
	{
		TSlot& rSlot = *itor;
		auto x = float(rSlot.ixPosition);
		auto y = float(rSlot.iyPosition);

		if (scaling != 0) {
			const auto xposoff = (rSlot.ixPosition * (1.0 - scaling)); x -= xposoff;
			const auto yposoff = (rSlot.iyPosition * (1.0 - scaling)); y -= yposoff;
			const auto woffx = (32 * (1.0 - scaling)) / 2; x -= woffx;
			const auto hoffy = (32 * (1.0 - scaling)) / 2; y -= hoffy;
		}

		if (rSlot.pSlotButton)
		{
			rSlot.pSlotButton->SetPosition(x + 1, y + 19 * scaling);
		}

		if (rSlot.pSignImage)
		{
			rSlot.pSignImage->SetPosition(x - 7 * scaling, y + 10 * scaling);
		}

		if (rSlot.pInstance)
		{
			rSlot.pInstance->SetPosition(m_rect.left + x, m_rect.top + y);
			rSlot.pInstance->Render(scaling);
		}
		...
		if (!rSlot.isItem)
		{
			if (IS_SET(rSlot.dwState, SLOT_STATE_ALWAYS_RENDER_COVER))
			{
				...
				rSlot.pCoverButton->SetPosition(x, y);
				rSlot.pCoverButton->RenderButton(scaling);
			}
		...
		if (rSlot.fCoolTime != 0.0f)
		{
			...
			int32_t iCellY = rSlot.iyCellSize * scaling;
			int32_t iCellX = rSlot.ixCellSize * scaling;

			iCellX = min(iCellX, iCellY);

			auto fRadius = iCellX * 0.5f;
			auto fxCenter = m_rect.left + x + (iCellX * 0.5f * scaling);
			auto fyCenter = m_rect.top + y + (iCellY * 0.5f * scaling);

			// if (rSlot.bIsInverseCoolTime)
			// {
				// CPythonGraphic::Instance().RenderCoolTimeBoxInverse(
			...
			// {
				// CPythonGraphic::Instance().RenderCoolTimeBoxColor(
					...
					// fRadius,
					// fPercentage,
					// rSlot.sCoolTimeColor.r,
					// rSlot.sCoolTimeColor.g,
					// rSlot.sCoolTimeColor.b,
					// rSlot.sCoolTimeColor.a);
			// }


			// if (fcurTime - rSlot.fStartCoolTime >= rSlot.fCoolTime && !rSlot.bIsInverseCoolTime)
		...
		{
			int ix = (rSlot.byxPlacedItemSize * ITEM_WIDTH * scaling) + x - 4 * scaling;
			int iy = (rSlot.byyPlacedItemSize * ITEM_HEIGHT * scaling) + y - 12 * scaling + 2;
		...
		{
			rSlot.pFinishCoolTimeEffect->SetPosition(x, y);
			rSlot.pFinishCoolTimeEffect->Update();
			rSlot.pFinishCoolTimeEffect->SetScaling(scaling);
...
void CSlotWindow::RenderSlotBaseImage()
{
	...
	for (TSlotListIterator itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		m_pBaseImageInstance->SetPosition(m_rect.left + x, m_rect.top + y);
		m_pBaseImageInstance->Render(scaling);
	}
}
...
void CSlotWindow::OnRenderPickingSlot()
{
	...
	auto x = float(pSlot->ixPosition);
	auto y = float(pSlot->iyPosition);

	if (scaling != 0) {
		const auto xposoff = (pSlot->ixPosition * (1.0 - scaling)); x -= xposoff;
		const auto yposoff = (pSlot->iyPosition * (1.0 - scaling)); y -= yposoff;
		const auto woffx = (32 * (1.0 - scaling)) / 2; x -= woffx;
		const auto hoffy = (32 * (1.0 - scaling)) / 2; y -= hoffy;
	}
	...
	CPythonGraphic::Instance().RenderBar2d(m_rect.left + x,
		m_rect.top + y,
		m_rect.left + x + pSlot->ixCellSize * scaling,
		m_rect.top + y + pSlot->iyCellSize * scaling);
}
	...
	for (TSlotListIterator itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		if (rSlot.dwState & SLOT_STATE_LOCK)
		{
			CPythonGraphic::Instance().RenderBar2d(
				m_rect.left + x,
				m_rect.top + y,
				m_rect.left + x + rSlot.ixCellSize * scaling,
				m_rect.top + y + rSlot.iyCellSize * scaling);
		}
	}
}
#else
...
void CSlotWindow::SetSwapItem(BOOL bFlag)
{
	m_isSwapItem = bFlag;
}
#endif
...
void CSlotWindow::ActivateSlotEffect(DWORD dwSlotIndex, float r, float g, float b, float a)
{
	TSlot* pSlot;
	if (!GetSlotPointer(dwSlotIndex, &pSlot))
		return;
...
void CSlotWindow::DeactivateSlotEffect(DWORD dwSlotIndex)
{
	TSlot* pSlot;
	if (!GetSlotPointer(dwSlotIndex, &pSlot))
		return;
