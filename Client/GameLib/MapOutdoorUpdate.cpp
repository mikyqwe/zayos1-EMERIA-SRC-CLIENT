...
#endif
	//__UpdateGarvage();	//RENDER_TERRAIN_FIX
#ifdef __PERFORMANCE_CHECKER__
...
struct FGetShadowReceiverFromHeightData
{
	...
	FGetShadowReceiverFromHeightData(float fFromX, float fFromY, float fToX, float fToY) :
		m_fFromX(fFromX), m_fFromY(fFromY), m_fToX(fToX), m_fToY(fToY), m_fReturnHeight(0), m_bReceiverFound(false)
	{
		m_dwCheckCount=0;
...
struct PCBlocker_SInstanceList
{
	...
	PCBlocker_SInstanceList(PCBlocker_CDynamicSphereInstanceVector* pkDSIVector)
		: m_pkDSIVector(pkDSIVector), m_dwInstCount(0), m_dwBlockerCount(0), m_dwBlockerOverCount(0)
	{
