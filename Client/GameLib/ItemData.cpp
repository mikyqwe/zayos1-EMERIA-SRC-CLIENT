...
D3DXVECTOR3 CItemData::GetItemScalePosition(DWORD dwPos)
{
	D3DXVECTOR3 p;
	p.z = m_ScaleTable.tInfo[dwPos].fPositionZ;
	p.y = m_ScaleTable.tInfo[dwPos].fPositionY;
	p.x = m_ScaleTable.tInfo[dwPos].fPositionX;
	return p;
}
...
const char* CItemData::GetUseTypeString() const
{
	...
	switch (GetSubType())
	{
		...
		case USE_CHANGE_LV_P:
			return DEF_STR(USE_CHANGE_LV_P);
#endif
	}
